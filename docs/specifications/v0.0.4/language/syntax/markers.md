...
**See Also:**
- [Serial Load Block Feature](../language/advanced/serial-load-block.md) - Complete feature documentation
...
**Indentation:** Each separate `[y]` starts a new independent conditional chain
...
**Indentation levels:**
1. `[y] $role =? "admin"` - Top level condition
2. Code indented once - Executes when admin
3. Nested `[y]` indented once - Sub-condition within admin block
...
[y] $user_authenticated =? #True
[&] $session_valid =? #True
   // User is authenticated with valid session

   [y] $resource_owner =? $current_user_id
...
[y] $option =? "A"
   [r] $result << 1
[y] $option =? "B"           // Unclear if independent or related
   [r] $result << 2
```
...
[y] $result1.state =? :pg.state.faulted
   [r] $errors << "Operation1 failed"

[y] $result2.state =? :pg.state.faulted
   [r] $errors << "Operation2 failed"

[y] $result3.state =? :pg.state.faulted
...
[y] $primary_error.state =? :pg.state.faulted
   [r] $data << |FallbackSource""
   [z][!] *! >> $fallback_error

   // If both failed, use default
   [y] $fallback_error.state =? :pg.state.faulted
      [r] $data << #DefaultData
```

**How it works:**
1. `[z]` tries primary source (may fail → Faulted + error object)
2. `[y]` checks if primary failed
3. `[r]` sequentially tries fallback
4. Nested `[y]` checks if fallback also failed
...
[y] $result.state =? :pg.state.faulted
   [r] $log_level << "ERROR"
...
- [Loop System](../language/advanced/loop-system.md) - `[~]` and `[*]` in detail
- [Serial Load Block](../language/advanced/serial-load-block.md) - `[s]` in detail
