...
[y] $age >? 18                     ✅ One marker, one expression
...
[W] |W.Polyglot.Scope
   ...
   [y] $input =? "test"            // 3-space indent (nested in wrapper)
...
[y] $order.status =? "pending"       // Principle 4: Hierarchy ($order.status)
...
**See:** [Inline Pipelines](../language/advanced/inline-pipelines.md) for complete details on `%Inline.FormattedString` and formatter pipelines
