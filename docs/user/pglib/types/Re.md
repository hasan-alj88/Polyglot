...
`#Re` is a pglib type representing a compiled regular expression pattern. It stores the regex source text and is validated at compile time by the native `PgRegex` class — the compiler parses the pattern string using an actual regex engine, not regex-on-regex validation.
