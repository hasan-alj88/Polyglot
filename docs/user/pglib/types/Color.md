...
`#Color` is a pglib type representing a color as RGBA components. It stores red, green, blue, and alpha channels as integers (0-255 each). Color validation requires range checking and named color lookup, so `#Color` is backed by a native `PgColor` class.
...
- [[pglib/constructors/Color\|$Color constructor]] -- compile-time color construction
