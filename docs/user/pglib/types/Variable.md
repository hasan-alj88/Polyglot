...
The `.type` field references the corresponding language branch of [[pglib/types/NativeType|#NativeType]]. This constrains `.type` to only valid types for the active language -- a `:Python` variable cannot have `.type#NativeType.Rust`.
...
See [[pglib/pipelines/Variable/Convert|-Variable.Convert]] for the pipeline definition and [[technical/algorithms/bridge-conversion|Bridge Conversion Algorithm]] for the full algorithm.
...
- [[pglib/pipelines/Variable/Convert|-Variable.Convert]] -- pairwise type conversion
