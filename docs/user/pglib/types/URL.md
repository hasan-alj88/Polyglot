...
`#URL` is a pglib type representing a URL (RFC 3986). It decomposes a URL string into its scheme, host, port, path, query, and fragment components. URL parsing requires an RFC 3986-compliant parser — not regex — so `#URL` is backed by a native `PgUrl` class.
...
- [[pglib/constructors/URL\|$URL constructor]] -- compile-time URL construction
