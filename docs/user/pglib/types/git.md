...
Pglib struct providing typed payloads for Git event triggers (`-T.Git.*`). All subtypes are nested under a single `{#} #Git` definition. Enum subtypes carry `[#] ##Enum` schema classification.

See [[pglib/pipelines/T/INDEX|-T.* Trigger Pipelines]] for the trigger family that produces these types.
...
- [[pglib/pipelines/T/INDEX|-T.* Trigger Pipelines]] — triggers that produce `#Git` payloads
- [[pglib/pipelines/T/Git.Hook|-T.Git.Hook]] — local hook transport trigger
- [[pglib/pipelines/T/Git.Push|-T.Git.Push]] — push event trigger
- [[pglib/pipelines/T/Git.PR|-T.Git.PR]] — pull request event trigger
- [[pglib/pipelines/T/Git.Tag|-T.Git.Tag]] — tag creation trigger
