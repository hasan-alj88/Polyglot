...
**Native pipeline conversion pattern:** Like `$Dur`, the `$URL` constructor delegates to a native pipeline (`-URL.Decompose`) that performs both validation and field extraction. This pipeline is a pglib-internal compile-time primitive — it is invoked during constructor evaluation, not at runtime.
...
- [[pglib/pipelines/URL.Parse|-URL.Parse]] -- runtime URL string parsing
- [[pglib/types/URL|#URL type]] -- URL type definition
