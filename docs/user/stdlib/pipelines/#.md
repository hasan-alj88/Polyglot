...
Types are data trees (GT-1). The `<#type` syntax passes a type definition's `%` metadata tree as pipeline input — extending `<#` from `{M}` macro type inputs to `{=}` pipeline IO. Works with any tier: `#Config` (struct), `##Scalar` (schema), `###Enum` (property). See [[types#`<#type` in Pipeline IO]].
...
## Base Parsers (Compiler Intrinsics)
...
See also: [[collections#Expand Operators]], [[types#Macro-Generated Types]]

## Permissions

Validation and extraction pipelines perform pure computation — no `[_]` permission declarations needed. Base parsers are compiler intrinsics and have no permission requirements.
