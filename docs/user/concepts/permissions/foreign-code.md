...
Pipelines using `[C]` foreign code blocks ([[blocks#Foreign Code|u:Foreign Code]]) and `-Run.*` pipelines ([[jm3lib/pipelines/Run/INDEX|u:Run pipelines]]) interact with permissions as follows:
...
<!-- @u:jm3lib/pipelines/Run/INDEX -->
Each `-Run.*` mode ([[jm3lib/pipelines/Run/INDEX|u:Run pipelines]]) has a different level of AST analysis:
