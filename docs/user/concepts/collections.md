...
`#Dataframe` is a row-oriented table generated by the `{M} #Dataframe` type macro. Each row is a `#Map` keyed by column enum, and the dataframe itself is an array of those maps. Two overloads: compile-time safe `(<#, <#)` and runtime flexible `(<, <)`. Uses `##` composition (`##Contiguous`, `##Rectangular`, `##Ordered`) — no `<~` inheritance. See [[stdlib/types/collections]] for the full macro definition.
...
For entire-column extraction, use `=#.Column` pipeline (see [[stdlib/pipelines/#]]).
