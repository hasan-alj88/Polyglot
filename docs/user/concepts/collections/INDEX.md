...
Every collection in Aljam3 is a tree. `##Map` is the universal flat key-value collection schema. `##Array` is a `##Map` variant where keys are contiguous integer enums. `##Serial` drops all schema constraints — it accepts any compilable tree.

| Schema | Is a | Key type | Key structure |
|------|------|----------|--------------|
...
Collections are **assembled at once** using collect operators (`*` prefix) — not incrementally added to at runtime. Collections are populated via the unified collector (`*Collect`) and are structurally complete after collection.

## Sub-Pages

| File | Covers |
|------|--------|
| [[map]] | ##Map base collection schema |
| [[array]] | ##Array map variant schema |
| [[serial]] | ##Serial unconstrained tree schema |
