...
Pipelines break into **jobs** at IO boundaries — each `[-]`, `[=]`, or `[b]` marker creates a job. `[Q]` lines can be nested under these markers to scope queue conditions to that specific job and its sub-jobs (the branch subtree), rather than the entire pipeline.
...
In this example, all jobs inherit `-Q.Default`. The `-Transform` branch additionally pauses when RAM drops below 2048 MB — this condition only affects that `[=]` branch and any sub-jobs it spawns, not the `-Validate` branch or the `[-]` jobs.
