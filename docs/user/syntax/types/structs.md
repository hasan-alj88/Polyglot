...
Stdlib structs with value fields include `#path`, `#Queue`, and `#DateTime` (see [[stdlib/types/structs|stdlib structs]] and [[stdlib/types/datetime/INDEX|datetime]]). Enum types (`#Boolean`, `#OS`, `#PipelineStatus`, `#VarState`) are also `{#}` structs but documented separately — see [[stdlib/types/enums|enums]] and [[stdlib/types/boolean|boolean]]. User-defined structs follow the same rules.
