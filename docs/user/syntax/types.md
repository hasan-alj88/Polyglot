...
9. **GT-9: `{M}` macros replace generic type parameters** — parameterized types use macros to generate `{#}` definitions at compile time
...
The `<` operator also appears inside `{M}` macro definitions as a parameter input marker (`[#] <Param`). See [[#Macro-Generated Types]] for details.
...
Scalar subtypes are `##` schemas — regex constraints on `#String`, not standalone `#` structs. They live at `%##` on the metadata tree, generated by the `{M} #String.Subtype` macro (see [[#Macro-Generated Types]]). Each `{#} ##Name` definition invokes `[M] #String.Subtype` to inherit `#String`'s schema and set `.re` to a specific regex:
...
The `{M} #String.Subtype` macro generates the `[#] <~ #String` inheritance, `[#] << ##Scalar`, alias registration, and `.re` override internally. See [[#Macro-Generated Types]] for the macro definition.
...
- `dataframe` — row-oriented table. A `#Dataframe` is an array of maps — each row is a `#Map`, the array holds rows. Row access uses `<` (array index), column access chains a second `<`: `$df<row<column`. See [[collections]].
...
└── #String (foundation — .string + .re) [##Scalar, ###Value] (Layer 0)
    ├── ##Int (.re = signed integers) — macro-generated via {M} #String.Subtype (Layer 1)
    ├── ##UnsignedInt (.re = non-negative integers) (Layer 1)
    ├── ##Float (.re = decimals) (Layer 1)
    ├── ##Sci (.re = scientific notation) (Layer 1)
    ├── ##Eng (.re = engineering notation) (Layer 1)
    ├── ##Dimension (.re = dimension values) (Layer 1)
    ├── ##KeyString (.re = syntax-safe keys) (Layer 1)
    ├── ##NestedKeyString (.re = alias-safe paths) (Layer 1)
    ├── ##CommaSeparatedList (.re = comma-separated identifiers) (Layer 1)
    ├── ##DataTypeString (.re = valid {x} definition names) (Layer 1)
