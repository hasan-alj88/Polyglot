...
| `{$}` | Constructor definition — produces compile-time-guaranteed Final values with no error surface. Two overload forms: string-parsing (`($)` regex captures mapped to `[$]` target type) and native pipeline (jm3lib only, `[-]` infallible calls). Prefix symmetry: `{#}` → `#Type`, `{-}` → `-Pipeline`, `{$}` → `$Constructor`. Metadata at `%$`. See [[constructors]] |
