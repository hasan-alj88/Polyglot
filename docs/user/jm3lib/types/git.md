...
Jm3lib struct providing typed payloads for Git event triggers (`-T.Git.*`). All subtypes are nested under a single `{#} #Git` definition. Enum subtypes carry `[#] ##Enum` schema classification.
