...
|------|-----------|-------------|
| Epic 1 | Lexer & Parser (v0.0.3) ✅ | FR1, FR2, FR6, FR7, FR9 |
...
| Epic 13 | v0.0.4 Syntax Migration 🔄 | FR1, FR2 (v0.0.4 compliance) |

---

## Epic 1: Lexer & Parser (v0.0.3)

**Epic Goal:** Implement tokenization and parsing of `.pg` files into Abstract Syntax Tree (AST), enabling developers to write Polyglot pipelines using the v0.0.3 syntax specification.

**Epic Status:** ✅ **COMPLETE** (Stories 1.1-1.8 finished December 2025)
...
- Technology: Rust with recursive descent parser (pest/nom not needed for v0.0.3 complexity)
- Output: AST representation ready for IR generation
- Error handling: thiserror for library-level errors

**Note:** This epic implements v0.0.3 syntax. v0.0.4 syntax (with `$` variables, indentation-based nesting, `{|}` blocks) will be implemented in future epic (Epic 1B or Epic 13).
...
- Epic 12: Testing & QA
- Epic 13: v0.0.4 Syntax Migration (Planned Q2 2026)

---

## Epic 13: v0.0.4 Syntax Migration

**Epic Goal:** Migrate lexer and parser from v0.0.3 to v0.0.4 syntax, implementing all 33 language features defined in the v0.0.4 specification.

**Epic Status:** 📋 **ARCHITECTURE COMPLETE** - Ready for Implementation (Dec 29, 2025)

**Business Value:** Enable modern syntax features (indentation-based nesting, metadata system, pipeline composition) that reduce code verbosity by 44% and add powerful capabilities for production automation.

**Architecture Context:**
- Crate: `polyglot-lexer` and `polyglot-parser` (major updates)
- Technology: Extend existing Rust implementation
- **Architecture Documents:** ✅ Complete
  - [Lexer Architecture v0.0.4](./architecture/lexer-architecture-v0.0.4.md) - 124 token types, state machine design
  - [Parser Architecture v0.0.4](./architecture/parser-architecture-v0.0.4.md) - Complete AST design, recursive descent
- Major Changes: Indentation parser, new AST nodes for loops/metadata/composition, 3 new tokens
- Breaking: v0.0.3 code will not compile without migration

**Architecture Highlights:**
- **124 Total Token Types** (121 original + 3 new: PipelineComposition, Input, Output)
- **State Machine Lexer** with 6 states (Initial, InString, InPath, InComment, InBlockMarker, InParenIO)
- **3-Character Lookahead** for maximal munch disambiguation
- **Indentation Tracking** (loop bodies only - whitespace-insensitive elsewhere)
- **Recursive Descent Parser** with explicit error recovery
- **Three-Phase Resolution** (current file → same package → registry)
- **Complete AST Design** with all v0.0.4 constructs

**Implementation Stories:**
- ✅ **Story 13.1:** [Complete v0.0.4 Lexer Implementation](./stories/13-1-lexer-v0.0.4-complete.md)
  - 124 token types with 3 new tokens (|>, <param, >param)
  - Pipeline composition disambiguation (|> vs |Pipeline)
  - Input/Output parameter tokenization
  - Metadata prefix `%` and reserved indication `;`
  - Indentation tracking (INDENT/DEDENT tokens)
  - 2-3 weeks estimated effort

- ✅ **Story 13.2:** [Complete v0.0.4 Parser Implementation](./stories/13-2-parser-v0.0.4-complete.md)
  - Pipeline composition parsing
  - Loop statements with indentation-based bodies
  - Enum blocks with serial loads
  - Error blocks with pattern matching
  - Trigger I/O wiring
  - Type validation (nested pg. prefix)
  - Three-phase pipeline resolution
  - 3-4 weeks estimated effort

- 📋 **Story 13.3:** IR Generation v0.0.4 Updates (TBD)
  - Update 3-IR structure for new constructs
  - Pipeline composition IR
  - Loop IR with unpack/pack operators
  - Enum serial load IR

- 📋 **Story 13.4:** v0.0.3 → v0.0.4 Migration Tool (TBD)
  - Automated syntax migration
  - Breaking change detection
  - Migration guide generation

- 📋 **Story 13.5:** Integration Testing & Documentation (TBD)
  - End-to-end v0.0.4 examples
  - Performance benchmarks
  - User documentation updates

**Dependencies:**
- **Prerequisite:** Epic 1 (v0.0.3) ✅ Complete
- **Blocks:** Epic 2-12 can proceed with v0.0.3 syntax, but will need updates for v0.0.4

**Previous Progress (Dec 17, 2025):**
- ✅ Lexer operators: `<<<`, `>>>`, `in?`, `in!?`, `=!?`, `>!?`, `<!?`, `>=!?`, `<=!?`, `re!?`, `|>`
- ✅ Lexer fixes: `>=?` and `<=?` (were incorrectly `=>?` and `=<?`)
- ✅ Marker rename: `BlockBoolGroup` → `BlockSubfield`
- ⚠️ Failing tests: 3 parser tests broken by lexer changes (needs fix in Story 13.1)
- ❌ Missing lexer features: `%` metadata prefix, `;` reserved indication (now documented in Story 13.1)
- ❌ Parser: All v0.0.4 syntax (now fully architected in Story 13.2)

**NEW Architecture Completion (Dec 29, 2025):**
- ✅ Winston (Architect): Complete lexer architecture with 124 token types
- ✅ Winston (Architect): Complete parser architecture with full AST design
- ✅ Winston (Architect): Disambiguation algorithms for all multi-character operators
- ✅ Winston (Architect): Indentation handling strategy (loop bodies only)
- ✅ Winston (Architect): Three-phase pipeline resolution design
- ✅ John (PM): Detailed implementation stories created (13.1, 13.2)
- ✅ Implementation-ready with clear acceptance criteria and technical guidance

**Estimated Total Effort:** 5-7 weeks (2-3 weeks lexer + 3-4 weeks parser)

**Target:** Q1 2026 (architecture complete, implementation can begin immediately)

**Next Actions:**
1. Begin Story 13.1 (Lexer v0.0.4 Implementation)
2. After Story 13.1 complete, begin Story 13.2 (Parser v0.0.4 Implementation)
3. Create Story 13.3-13.5 as implementation progresses
