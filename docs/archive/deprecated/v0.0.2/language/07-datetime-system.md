...
**IMPORTANT:** Polyglot does NOT have comparison operators (`<`, `>`, `==`, etc.). Use range notation with the match operator `?>` instead.

```polyglot
// Compare dates using range notation
...
[r] |DT.IsBetween
[<] .date: pg\dt << DT"2024-01-15"
[<] .start: pg\dt << DT"2024-01-01"
[<] .end: pg\dt << DT"2024-01-31"
[>] .in_range: pg\bool >> is_in_january
```
