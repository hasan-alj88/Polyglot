...
original_path: user/language/12-bnf-grammar.md
---
...
**Status:** In Progress - Updated for registry/execution marker split

---

## Overview

This document provides **two formal BNF grammars** for the Polyglot language v0.0.3, corresponding to the two compilation phases:

1. **Surface Syntax Grammar (Phase 1)** - Compact form users write
2. **Canonical Syntax Grammar (Phase 2)** - Full render form after wrapper expansion

### Compilation Pipeline

```
User Source Code (.pg files)
      ↓
[Phase 1: Parse]
   Uses: Surface Syntax Grammar
   Recognizes: Registry markers {}, Execution markers [], Nested markers \marker\
   Output: Surface AST
      ↓
[Phase 2: Transform]
   - Wrapper expansion
...
[Phase 3: Validate]
   ...
   Output: Validated IR (JSON format)
      ↓
[Phase 4: Registration]
   Store IR in PostgreSQL database
```

**Important:** This grammar reflects v0.0.3 syntax with:
- **Registry/Execution marker split**: Curly braces `{}` for definitions, square brackets `[]` for execution
- **Nested marker syntax**: Backslash prefix/suffix `\marker\` for nested operations
- **Wrapper system**: Replaces macro system with `{W}` wrapper definitions
...
```bnf
<program> ::= <registry-declaration-block>
              { <top-level-element> }

/* CRITICAL: Every .pg file MUST start with exactly ONE registry declaration block
   This declares the registry identity and dependencies */

<registry-declaration-block> ::= "{@}" <registry-spec>
                                 [ <alias-declaration> ]
                                 { <dependency-declaration> }
                                 "{x}"

<registry-spec> ::= "@" <scope> "::" <namespace> "." <name> ":" <version>

<scope> ::= <identifier>  /* e.g., Local, Community.username, Official */
...
<alias-declaration> ::= "[A]" <prefix> <identifier>

<prefix> ::= "@"  /* Registry alias */
           | "#"  /* Enumeration alias */
           | "|"  /* Pipeline alias */
           | "!"  /* Error alias */
           | "|W."  /* Wrapper alias */

<dependency-declaration> ::= "[.]" "@" <dep-alias> "<<" <registry-spec>

<dep-alias> ::= <identifier>
...
```bnf
<pipeline-definition> ::= "{|}" "|" <pipeline-path>
                          { <pipeline-element> }
                          "{x}"

<pipeline-path> ::= "Pipeline." <identifier> { "." <identifier> }
...
<constant-input> ::= <field-name> ":" <type> "<<" <expression>

<default-input> ::= <field-name> ":" <type> "<~" <expression>

<output-declaration> ::= "[>}" <output-spec>

<output-spec> ::= <field-name> ":" <type>
                | "#None"

<wrapper-scope-input> ::= "[{]" <field-name> ":" <type>

<macro-scope-output> ::= "[}]" <field-name> ":" <type>

/* Note: Keywords removed:
   - "Fixed" → use [i] with << (constant input)
   - "Default" → use [i] with <~ (default input)
   - "Exposed" → use [}] scope output for macro-exported variables

   Input variations:
   - [i] .var: Type         - Required input (Declared)
   - [i] .var: Type <~ val  - Default input (DefaultReady)
   - [i] .var: Type << val  - Constant input (Ready)
*/
```
...
- `Fixed` → `[i]` with `<<` for constant input
- `Default` → `[i]` with `<~` for default input
- `Exposed` → `[}]` scope output for macro-exported variables
