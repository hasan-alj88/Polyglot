...
original_path: user/async-centric-paradigm.md
---
...
[p] .a:pg.string << |U.HTTP.Get"/api/a"  // Starts immediately
[p] .b:pg.string << |U.HTTP.Get"/api/b"  // Starts immediately
...
[r] .result:pg.serial << "{.both[0]}{.both[1]}"
```
...
{|} |Pipeline.ParallelDataProcessing
[<} .db_connection:pg.string <~ #MyDB.ConnectionString
[t] |T.Call
[>} .final:pg.array{pg.serial}
...
\<\ <sql << |SQL"SELECT * FROM table_a"
\>\ >result >> .data_a

[p] |U.DB.Postgresql.Query.SQL
\<\ <sql << |SQL"SELECT * FROM table_b"
\>\ >result >> .data_b
...
\<\ <data1 << .data_a
\<\ <data2 << .data_b
\>\ >processed1 >> .processed_a
\>\ >processed2 >> .processed_b

// Wait for file data - auto-awaits when used
