...
**Epic Status:** 🔄 **IN PROGRESS** - Lexer ~75% complete (operators done), Parser not started
...
- Major Changes: Indentation parser, new AST nodes for loops/metadata/composition
- Breaking: v0.0.3 code will not compile without migration

**Planned Stories (TBD):**
- Story 13.1: Complete v0.0.4 Lexer (`%` metadata, `;` reserved indication, validate `$`)
- Story 13.2: Indentation-Based Nesting Parser
- Story 13.3: Loop System AST (`[~]`, `[*]`, `[v]` markers)
- Story 13.4: Pipeline Composition Parser (`|>` operator)
- Story 13.5: Metadata System Parser (`%` prefix, metadata blocks)
- Story 13.6: Inline Pipeline Parsing
- Story 13.7: Match Expression & Early Return
- Story 13.8: New Markers (`[s]` serial load, boolean markers)
- Story 13.9: v0.0.3 → v0.0.4 Migration Tool
- Story 13.10: Integration Testing & Documentation

**Dependencies:**
- **Prerequisite:** Epic 1 (v0.0.3) ✅ Complete
- **Blocks:** Epic 2-12 can proceed with v0.0.3 syntax, but will need updates for v0.0.4

**Current Progress (Dec 17, 2025):**
- ✅ Lexer operators: `<<<`, `>>>`, `in?`, `in!?`, `=!?`, `>!?`, `<!?`, `>=!?`, `<=!?`, `re!?`, `|>`
- ✅ Lexer fixes: `>=?` and `<=?` (were incorrectly `=>?` and `=<?`)
- ✅ Marker rename: `BlockBoolGroup` → `BlockSubfield`
- ⚠️ Failing tests: 3 parser tests broken by lexer changes (needs fix)
- ❌ Missing lexer features: `%` metadata prefix, `;` reserved indication
- ❌ Parser: All v0.0.4 syntax (indentation, `$`, `{|}`, etc.)

**Target:** Q2 2026 (aligned with PRD specification timeline)
