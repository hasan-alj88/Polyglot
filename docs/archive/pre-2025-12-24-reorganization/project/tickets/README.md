...
3. Labels automatically map (see `docs/Agile/itil-config.yaml`)
...
```bash
grep -r "priority: P1" docs/Agile/tickets/incidents/
```

**Find open tickets:**
```bash
grep -r "status: in_progress" docs/Agile/tickets/
```

**Find tickets assigned to Amelia:**
```bash
grep -r "assigned_to: Amelia" docs/Agile/tickets/
```
...
**Generated Reports Location:** `docs/Agile/tickets/reports/`
