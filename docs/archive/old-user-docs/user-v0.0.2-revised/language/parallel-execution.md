...
### Pattern 4: Result Collection (Sequential with Mutable Accumulator)

**Use Case:** Collect results from sequential iterations using a mutable accumulator
...
**Key Points:**
- Use `pg.mutable\type` for accumulator
- Unpack operator: `~ForEach` (like a pipeline call)
- String interpolation for concatenation
- Final conversion from accumulated string to desired type

---

### Pattern 5: Result Collection (Parallel with Auto-Aggregation)

**Use Case:** Collect results from parallel operations with automatic aggregation
...
**Key Points:**
- `[Y] |Y.JoinAll` automatically aggregates results into array
- Input variables passed with `[<]` to join
- Output array type inferred from individual result types
- No manual accumulation needed

---

### Pattern 6: Hybrid Collection (Parallel Groups + Sequential Aggregation)

**Use Case:** Collect results from multiple parallel groups, then aggregate sequentially
...
// Process each dataset in parallel groups
[r] ~ForEach
[<] .datasets
[>] .dataset
...
[o] .all_results: pg.mutable\array{pg\string}
[X]
```

**Key Points:**
- Combines sequential iteration with parallel execution
- Each iteration has internal parallelism
- Results aggregated at each level
- Flexible pattern for complex workflows
...
- [Complete Syntax Reference](syntax-complete.md) - `[p]` and `[Y]` syntax
- [Block Markers](block-markers.md) - `[p]` and `[Y]` details
- [Operators](operators.md) - `>>` operator semantics
- [Expansion Operator](expansion-operator.md) - `[~]` in parallel blocks
