...
**Problem Summary:**
- `reference/type-system.md` says "Maps have been removed from Polyglot"
...
```polyglot
language\type          // All types: pg\int, pg\string, pg\uint, pg\path
```
...
- **Consistent** with hierarchical dot notation used throughout language
...
[i] .api_key: pg\string << "secret-key-123"     // Fixed constant
...
**✅ UPDATED (v0.0.2):** No "Fixed" or "Default" keywords exist.

**Correct Syntax:**
```polyglot
// ✅ Constant (immediately Ready)
[i] .api_key: pg\string << "secret-key-123"

// ✅ Default (optional, DefaultReady)
[i] .chunk_size: pg\int <~ 1024
...
1. Use `<<` (PUSH) for constants
2. Use `<~` (DEFAULT PUSH) for optional parameters with defaults
3. No value = required input (Declared state)
4. NO keywords ("Fixed" or "Default") exist in Polyglot
...
```polyglot
[i] .config: pg\serial << serial{
[^]  "host": "localhost",
[^]  "port": 8080
[^]}

[i] .settings: pg\array{pg\string} <~ array{
[^]  "option1",
[^]  "option2"
[^]}
```
...
**Action Items:**
**✅ COMPLETED (v0.0.2):**
- [x] Documented input declaration rules in v0.0.2
- [x] Updated all examples to follow canonical syntax (no Fixed/Default keywords)
- [x] Removed all instances of `[i] Fixed .x:` and `[i] Default .x:` keywords
- [x] Clarified: Use `<<` for constants, `<~` for defaults, no value for required
- [x] Added to syntax reference with clear examples of valid patterns
- [x] Documented `{}` as collection literal syntax (no keywords needed)
...
- Example: `[Compiler Architecture](../architecture/compiler.md)` works from epic file but not when rendered in different context
...
**Pattern:**
```markdown
[Link Text](/docs/user/planning/architecture/compiler.md)
```
...
- [ ] Use pattern: `/docs/user/section/file.md`
