...
- State-aware async coordination (variables transition through states)
...
1. [PUSH/PULL Operators](#pushpull-operators) ⭐ **Start Here!**
2. [Type Separator Syntax](#type-separator-syntax)
3. [Primitive Types](#primitive-types)
4. [Numeric Types](#numeric-types)
5. [String Type](#string-type)
6. [Boolean Type](#boolean-type)
7. [Path Type](#path-type)
8. [DateTime Type](#datetime-type)
9. [Collection Types](#collection-types)
10. [Serial Type](#serial-type)
...
### Understanding Polyglot's Async Model

**Critical Concept:** Polyglot is **async-centric**. We don't "assign values" - we **PUSH data INTO variables** and **PULL data FROM variables**. Variables transition through states as async operations complete.

---

### The Four Operators

| Operator | Name | Direction | State Transition |
|----------|------|-----------|------------------|
| `<<` | PUSH | Push INTO variable | → Ready |
| `>>` | PULL | Pull FROM variable | Waits if Pending |
| `<~` | DEFAULT PUSH | Push default INTO variable | → DefaultReady |
| `~>` | DEFAULT PULL | Pull default FROM variable | DefaultReady → Ready |
...
**What it does:** Pushes a value INTO a variable, making it **Ready** immediately.
...
// PUSH a constant value INTO .version
[r] .version: pg\string << "1.0.0"

// State: .version is now Ready
// Value: "1.0.0"
...
**State Transition:** Declared → **Ready**

---

### PULL Operator `>>`

**Syntax:** `.output: Type >> .target_variable`

**What it does:** Pulls a value FROM a source and stores it. **Waits if source is Pending.**
...
// PULL output FROM pipeline INTO .user_data
[r] |FetchUser
[<] .id: pg\string << .user_id
[>] .user: pg\string >> .user_data

// If pipeline is still running: WAITS
// When pipeline completes: .user_data becomes Ready
...
**State Behavior:**
- **Source is Ready:** Pull happens immediately, target becomes Ready
- **Source is Pending:** **WAITS** until source is Ready or Faulted
- **Source is Faulted:** Propagates error to target
- **Source is DefaultReady:** Transitions to Ready with default value (see below)
...
**What it does:** Pushes a **default value** that can be overridden once.

**Example:**
```polyglot
// Define enumeration with default timeout
[#] Config
[<] .timeout: pg\int <~ 30    // DEFAULT PUSH of 30 seconds
[X]
...
// Use default (not overridden)
[r] .config1: #Config << #Config
// .config1.timeout is in DefaultReady state with value 30

// Override default
[?] .override_timeout =? #Boolean.True
[~][r] .config2: #Config << #Config{.timeout: 60}
[~]// .config2.timeout immediately becomes Ready with value 60
...
**State Transition:** Declared → **DefaultReady**

---

### DEFAULT PULL Operator `~>`

**Syntax:** `.field: Type ~> .target_variable`

**What it does:** Pulls FROM a field that may have a default. **If DefaultReady, transitions to Ready immediately.**
...
[i] .settings: #Config << #Config

// PULL from .settings.timeout
[r] |SomeOperation
[<] .wait_time: pg\int << .settings.timeout

// What happens:
// 1. .settings.timeout is in DefaultReady state
// 2. PULL causes immediate transition: DefaultReady → Ready
// 3. .wait_time receives value 30
```

**State Behavior:**
- **DefaultReady:** **Immediately** transitions to Ready with default value
- **Ready:** Returns value immediately
- **Pending:** Waits until Ready

---

### When PULL Waits (Critical for New Users!)

**Rule:** When you PULL FROM a variable, Polyglot **automatically waits** if the variable is Pending.
...
[r] |FetchDataFromAPI
[<] .url: pg\string << .api_url
[>] .result: pg\string >> .api_data
// .api_data is now Pending (waiting for API response)

// Later in code...
[r] |ProcessData
[<] .input: pg\string << .api_data   // PULL from .api_data

// What happens:
// 1. .api_data is still Pending (API hasn't responded yet)
// 2. Polyglot WAITS automatically (no explicit await!)
// 3. When API responds, .api_data becomes Ready
// 4. PULL completes, |ProcessData starts executing
[>] .processed: pg\string >> .final_result

[o] .final_result: pg\string
[X]
```

**You never write `await`** - Polyglot does it automatically!

---

### Variable States Quick Reference

| State | Has Value? | Can PUSH? | What Happens on PULL? |
|-------|------------|-----------|----------------------|
| **Declared** | ❌ No | ✅ Yes | ❌ Error - no value to pull |
| **DefaultReady** | ✅ Default | ⚠️ Override once | ✅ Transitions to Ready, returns default |
| **Pending** | ❌ Waiting | ❌ No | ⏳ **WAITS** until Ready or Faulted |
| **Ready** | ✅ Yes | ❌ No | ✅ Returns value immediately |
| **Faulted** | ❌ Error | ❌ No | ❌ Propagates error |

---

### Common Patterns

**Pattern 1: PUSH a constant**
```polyglot
[|] Example
[i] #Pipeline.NoInput
[t] |T.Call
...
// .max_retries is Ready with value 3

[o] .max_retries: pg\int
[X]
```

**Pattern 2: PULL from pipeline output**
```polyglot
[|] Example
[i] .items: pg\array{pg\int}
[t] |T.Call
[W] |W.Polyglot.Scope

[r] |CalculateTotal
[<] .numbers: pg\array{pg\int} << .items
[>] .total: pg\int >> .final_total
// Waits for pipeline, then .final_total becomes Ready

[o] .final_total: pg\int
[X]
```

**Pattern 3: DEFAULT PUSH with optional override**
```polyglot
[#] Settings
[<] .debug: pg\bool <~ #Boolean.False    // Default: False
[X]

[|] Example
[i] .enable_debug: pg\bool
[t] |T.Call
[W] |W.Polyglot.Scope

[r] .app_settings: #Settings << #Settings{.debug: .enable_debug}
// .debug overridden based on input, immediately Ready

[o] .app_settings: #Settings
[X]
```

**Pattern 4: PULL from DefaultReady**
```polyglot
[#] Config
[<] .timeout: pg\int <~ 30
[X]
...
[r] |WaitFor
[<] .seconds: pg\int << .config.timeout
// PULL from DefaultReady: immediate transition to Ready
// |WaitFor receives 30
[>] .result: pg\bool >> .wait_result

[o] .wait_result: pg\bool
[X]
```
...
1. **No "assignment"** - We PUSH and PULL data
2. **PULL waits automatically** - If source is Pending, Polyglot waits (no `await` keyword!)
3. **DefaultReady → Ready on PULL** - Pulling from DefaultReady makes it Ready immediately
4. **Once Ready, no more PUSHes** - Ready variables cannot accept new values
5. **`<<` means INTO** - Push data INTO variable
6. **`>>` means FROM** - Pull data FROM source
...
// Default input (optional with default value)
[i] .timeout_seconds: pg\int <~ 30    // DEFAULT PUSH operator <~
```

**Operations:**
- Arithmetic: `+`, `-`, `*`, `/`, `%`
- Comparison: `>?`, `<?`, `=>?`, `=<?`, `=?`, `=!?` (all operators use `?` suffix)
- Range/Interval: `?[a, b]`, `?(a, b)`, `?[a, b)`, `?(a, b]` (mathematical notation)
- Bitwise: `&`, `|`, `^`, `~`, `<<`, `>>`
...
// Input with default
[i] .debug_mode: pg\bool <~ #Boolean.False  // DEFAULT PUSH operator <~
```
...
**See Also:** [DateTime System](datetime-system.md) for complete DT reference
...
**Description:** Serializable data structure with dynamic schema (keys can change at runtime)

**Key Characteristics:**
- Dynamic key-value pairs
- Runtime-flexible schema (unlike compile-time enumerations)
...
|---------|-------------|-------------|
| Schema | Dynamic (runtime) | Fixed (compile-time) |
...
|---------|----------------------|---------------------|
| Schema | Fixed (compile-time) | Dynamic (runtime) |
...
**See Also:** [Enumerations](enumerations.md) for complete enumeration reference
...
**See Also:** [Error Handling](error-handling.md) for complete error reference
...
| Category | Types | State Model | Schema |
|----------|-------|-------------|--------|
| **Primitives** | `pg\int`, `pg\uint`, `pg\float`, `pg\string`, `pg\bool` | Transitions to Ready | Fixed |
| **Special** | `pg\path`, `pg\dt` | Transitions to Ready | Fixed with reserved fields |
| **Collections** | `pg\array{T}`, `pg\set{T}` | Transitions to Ready | Fixed (element type) |
| **Dynamic** | `pg\serial` | Transitions to Ready | Runtime-flexible |
| **Enumerations** | `#EnumName` | Always Ready | Compile-time fixed |
| **Errors** | `!ErrorName` | Always Ready | Fixed (3 reserved + custom) |
...
- [Complete Syntax Reference](syntax-complete.md) - All syntax elements
- [Enumerations](enumerations.md) - Enumeration system details
- [Error Handling](error-handling.md) - Error type details
- [DateTime System](datetime-system.md) - DateTime type details

### Standard Library
- [Overview](../standard-library/overview.md) - Type conversion utilities
