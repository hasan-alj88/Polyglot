...
---
**⚠️ DEPRECATION NOTICE (2025-12-03):** This is an old backup file. It contains syntax violations including `[<]` using `>>` (wrong direction). For current operator documentation, see `docs/user/syntax/operators.md` (v0.0.6) and `docs/project/operator-directionality-rule-2025-12-03.md`.
---
...
7. [PUSH/PULL Operators](#pushpull-operators)
...
| `<<` | PUSH | Push INTO variable | `.x << value` |
| `>>` | PULL | Pull FROM variable | `.x >> output` |
| `<~` | DEFAULT PUSH | Push default INTO variable | `.x <~ 30` |
| `~>` | DEFAULT PULL | Pull FROM variable (handles defaults) | `.x ~> .y` |
...
[>] .processed: pg\string >> .step1

[r] |ValidateInput                  // Sequential pipeline call
[<] .data: pg\string << .step1
[>] .validated: pg\bool >> .is_valid

[r] |TransformOutput                // Sequential pipeline call
[<] .input: pg\string << .step1
[>] .output: pg\string >> .final
...
[r] |U.String.Format                // Standard library call
[<] .template: pg\string << "Hello {0}"
[<] .args: pg\array{pg\string} << .items
[>] .formatted: pg\string >> .result

[r] |U.Array.Length                 // Standard library call
[<] .array: pg\array{pg\string} << .items
[>] .length: pg\int >> .count
...
**Trigger declarations (in pipeline context):**
```polyglot
[|] ScheduledTask
[i] #Pipeline.NoInput
[t] |T.Daily                        // Trigger: runs daily
[W] |W.Polyglot.Scope
...
[|] FrequentTask
[i] #Pipeline.NoInput
[t] |T.Every.Minute                 // Trigger: runs every minute
[W] |W.Polyglot.Scope

[r] .check: pg\bool << #Boolean.True
[o] .check: pg\bool
[X]
```

**Wrapper declarations (in pipeline context):**
```polyglot
[|] PythonAnalysis
[i] .data: pg\string
[t] |T.Call
[W] |W.Python3.11                   // Wrapper: Python 3.11 runtime

[r] |AnalyzeData
[<] .input: py\str << .data
[>] .result: py\str >> .output
...
[r] |ProcessData                    // Pipeline call with parameters
[<] .input: pg\string << .data      // Input parameter
[<] .max_size: pg\int << 1024       // Input parameter
[>] .result: pg\string >> .output   // Output parameter
...
[>] .item                           // Output: each item

[~][r] |ProcessItem                 // Process each item
[~][<] .data: pg\string << .item
...
```polyglot
[|] StandardLibraryUnpack
[i] .data: pg\array{pg\string}
[t] |T.Call
[W] |W.Polyglot.Scope

// Array.ForEach - unpack operation
[p] ~ForEach                        // Unpack standard library operation
[<] .data
[>] .element

[~][r] |ProcessElement              // Process each element
[~][<] .item: pg\string << .element
...
[p] ~Enumerate                      // Unpack enumeration
[<] .config
[>] .field_name
[>] .field_value

[~][r] |ProcessField                // Process each field
[~][<] .name: pg\string << .field_name
[~][<] .value: pg\serial << .field_value
[~][>] .result: pg\string >> .processed
...
**Critical Concept:** Polyglot is async-centric. We don't "assign values" - we **PUSH data INTO variables** and **PULL data FROM variables**.

Polyglot has two distinct PUSH/PULL operators with opposite data flow directions:

| Operator | Name | Direction | State Behavior |
|----------|------|-----------|----------------|
| `<<` | PUSH | INTO variable | Makes variable Ready |
| `>>` | PULL | FROM source | **Waits if source is Pending** |

**Key Point for New Users:** When you PULL (`>>`) from a variable, Polyglot **automatically waits** if that variable is still Pending. No `await` keyword needed!
...
**State Behavior:**
- **Source is Ready:** Returns value immediately
- **Source is Pending:** **WAITS** until Ready or Faulted (automatic async wait!)
- **Source is DefaultReady:** Transitions to Ready immediately, returns default value
- **Source is Faulted:** Propagates error
...
### DEFAULT PUSH and DEFAULT PULL Operators

In addition to regular PUSH/PULL, Polyglot has operators for default values:

| Operator | Name | Direction | State Transition |
|----------|------|-----------|------------------|
| `<~` | DEFAULT PUSH | INTO variable | → DefaultReady |
| `~>` | DEFAULT PULL | FROM variable | DefaultReady → Ready |

---

#### DEFAULT PUSH `<~`

**Purpose:** Push a default value that can be overridden once.

**Example:**
```polyglot
[#] Config
[<] .timeout: pg\int <~ 30    // DEFAULT PUSH: 30 seconds
[<] .retries: pg\int <~ 3     // DEFAULT PUSH: 3 retries
[X]

// Use defaults
[i] .settings: #Config << #Config
// .settings.timeout is DefaultReady with value 30
// .settings.retries is DefaultReady with value 3

// Override one default
[i] .custom: #Config << #Config{.timeout: 60}
// .custom.timeout is Ready with value 60 (overridden)
// .custom.retries is DefaultReady with value 3 (still default)
```

**State:** Variable starts in **DefaultReady** state, can be overridden once.

---

#### DEFAULT PULL `~>`

**Purpose:** Pull from a field that may have a default value.

**State Behavior:**
- **If DefaultReady:** Immediately transitions to Ready, returns default value
- **If Ready:** Returns value immediately
- **If Pending:** Waits until Ready
...
// DEFAULT PULL from .settings.timeout
[r] |WaitOperation
[<] .duration: pg\int ~> .settings.timeout
// DefaultReady → Ready transition happens immediately
// |WaitOperation receives 30
```

**Note:** In practice, `~>` is rarely used explicitly. Most pulls handle DefaultReady automatically with `<<` or `>>`.
...
**See:** [Block Markers documentation](block-markers.md) for detailed boolean logic usage.
...
- [Complete Syntax Reference](syntax-complete.md) - All operators overview
- [Block Markers](block-markers.md) - Block elements that use operators
- [Type System](type-system.md) - Operator usage with types

### Advanced Features
- [Parallel Execution](parallel-execution.md) - `>>` in parallel blocks
- [Expansion Operator](expansion-operator.md) - `~` usage details
