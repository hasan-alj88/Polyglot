...
> Aljam3's developer experience is built around one loop: write, compile, fix, deploy. The compiler is not a gatekeeper — it is a collaborator that catches what you missed. This page explains why that trade-off pays off. See [[vision]] for the broader project context.
...
5. **Deploy** — The compiled [[glossary#Behavioral Contract|Behavioral Contract]] is registered with the [[glossary#Aljam3 Service|Aljam3 Service]]. What you deploy is exactly what the compiler verified.
...
Aljam3 inverts this. Every failure mode the compiler catches is a failure that will never reach production. The compile loop is longer than "write and run," but the deploy-to-incident loop is shorter — often zero, because the incidents were prevented at compile time.
...
Because Aljam3 pipelines do not have a `main()` function, testing operates differently than in traditional languages. Aljam3 encourages testing in a dedicated test environment where the Behavioral Contract remains 100% faithful to production. 

Crucially, **triggers should remain identical to production**. Instead of altering the code to test it, the Aljam3 platform provides a Debug/Testing mode that allows developers to manually *spoof* the environmental conditions—such as mocking the time, injecting file events, or artificially setting state flags. By spoofing the environment rather than the pipeline, the compiled contract tested is exactly the contract that will be deployed.
