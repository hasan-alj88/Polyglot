...
**Files Affected:**
- `examples/hello-world.md` - Examples 1, 2, 3 (helper), 5 (helpers), 6 (helpers), Common Patterns 1 & 2
...
**Files Affected:**
- `examples/hello-world.md` - ALL examples (Examples 1-6, all helpers, all patterns)
...
**Files Affected:**
- `examples/hello-world.md` - Examples 1, 2, 4 (line 243), 5, 6 helpers
...
**Files Affected:**
- `examples/hello-world.md` - Example 4 (ValidateAndGreet - line 237-243)
...
**Fix Required:**
- Document this compiler rule in language/syntax-complete.md
...
**Files Affected:**
- `examples/hello-world.md` - Uses `[?]` in Examples 3, 4, and Pattern 3
- `language/block-markers.md` - Missing `[?]` documentation
- `language/operators.md` - Missing `?>`, `==`, `!=`, `..`, comparison operators
- `language/syntax-complete.md` - Missing switch/conditional section
...
### Violation #9: Variable State Transition - Attempting to Re-Push to Ready Variable

**Severity:** CRITICAL
**Category:** Type System Violation
**Compiler Rule:** Variables in Ready state cannot accept new pushes

**Description:**
Polyglot is **async-centric** - variables transition through states (Declared → Pending → Ready/Faulted). Once a variable reaches **Ready** state, it cannot accept new pushes. This is a consequence of async completion, not a design goal.

**Invalid Code Pattern:**
```polyglot
// ✗ INVALID - Variable becomes Ready after first push
[r] .age_category: pg\string << ""        // Ready state (pushed empty string)
[~][r] .age_category: pg\string << "adult"  // COMPILER ERROR: Already Ready
```

**Correct Code Pattern:**
```polyglot
// ✓ CORRECT - Use switch to conditionally push before variable becomes Ready
[r] .age_category: pg\string              // Declared state

[?] .user_age ?> 18..
[~][<] .age_category << "adult"           // PUSH to variable
[~]

[?] .user_age ?> ..17
[~][<] .age_category << "minor"           // PUSH to variable
[~]

// After all conditional pushes, .age_category transitions to Ready
```

**Rationale:**
1. **Async-centric design** - Variables transition through states as async operations complete
2. **Thread safety** - Ready variables are immutable (consequence of completion)
3. **Predictability** - State transitions are explicit and trackable
4. **Explicit over implicit** - Use conditional pushes before Ready state

**Variable States:**
- **Declared**: No value yet, can accept pushes
- **Pending**: Async operation in progress
- **Ready**: Value available, no more pushes allowed (immutable)
- **Faulted**: Operation failed, has error info

**Files Affected:**
- All examples showing variable assignment
- Documentation on type system and variable states

**Fix Required:**
- Document variable state model as core language principle
- Show conditional push patterns for Declared variables
