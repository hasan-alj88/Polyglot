...
- Constant (immediately Ready): `[i] .{param}: {dtype} << {value}`
- Default (optional): `[i] .{param}: {dtype} <~ {value}`
- If no Input: `[i] #Pipeline.NoInput`

**Note:** No "Fixed" or "Default" keywords exist in Polyglot. Use operators:
- `<<` (PUSH) for constants
- `<~` (DEFAULT PUSH) for optional parameters with defaults
...
[i] .timeout: pg\int <~ 30            // DEFAULT PUSH operator <~
[t] |T.File.Modified
...
- [Complete Syntax Reference](../syntax-complete.md)
