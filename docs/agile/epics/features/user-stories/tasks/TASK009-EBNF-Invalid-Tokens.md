...
## Description
Ensure that any invalid syntax caught by the Lexer and passed into the stream as an unknown or invalid token directly triggers a formal Polyglot Grammar Error (PGE).
...
4. If an unknown Polyglot object (e.g. a prefix that wasn't successfully resolved into a valid identifier token) is detected, throw a PGE error.

## Acceptance Criteria
- [ ] `invalid marker -> PGE error` is strictly enforced.
- [ ] `unknown Polyglot object -> PGE error` is strictly enforced.
