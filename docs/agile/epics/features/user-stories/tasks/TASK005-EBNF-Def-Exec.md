...
## Description
Validate that all Definition (`{X}`) and Execution (`[X]`) markers in the token stream are followed by the exact grammatical sequences required by the Aljam3 EBNF specification.
...
4. If the sequence is broken, emit a PGE (Aljam3 Grammar Error) with the line and column number.
