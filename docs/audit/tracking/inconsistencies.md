...
| IC-003 | High | type-system | `#Dimension` described as "allows 0D" but regex `^[1-9][0-9]*D$` explicitly excludes 0D | [[pglib/types/types\|types.md]]:24 vs [[pglib/types/scalars\|scalars.md]]:117 | Resolved |
| IC-004 | High | wikilink | 12 broken `[[PGLIB]]` wikilinks — no PGLIB.md exists (see details below) | 7 files | Resolved |
| IC-005 | Medium | type-system | `#PipelineStatus`, `#VarState`, `#Boolean` listed as "pglib structs" but are enums | [[syntax/types/structs\|structs.md]]:17 vs [[pglib/types/enums\|enums.md]] | Resolved |
...
### IC-004: Broken `[[PGLIB]]` wikilinks (Resolved)

All 12 broken `[[PGLIB]]` references were rewritten to point to specific component files. The `metadata.md` references (lines 276–277) were removed during the AD-005 metadata split.
...
| IC-004 | High | 12 broken `[[PGLIB]]` wikilinks across 7 files | Rewrote each link to correct component: `[[pglib/INDEX\|Standard Library]]`, `[[pglib/pipelines/Path\|=Path]]`, `[[pglib/types/boolean\|#Boolean]]`, `[[pglib/INDEX#Pipeline Namespaces\|Wrappers]]` | 2026-03-30 |
