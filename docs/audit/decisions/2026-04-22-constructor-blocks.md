...
- User-definable; only jm3lib uses `[-]` pipeline calls inside `{$}`
...
- 9 jm3lib constructors created (issue #342): `$DT`, `$Path`, `$Re`, `$MIME`, `$Dur`, `$Ver`, `$URL`, `$IP`, `$Color`
...
- GitHub: #341 (design), #342 (jm3lib catalog), #343 (compile errors), #344 (docs transition)
