...
| Aljam3 Code | The .jm3 language for workflow orchestration | Not Python/Rust/JS code used within Aljam3 |
...
| Behavior Contract | The compiled output of a `.jm3` file: a signal map / signal graph that the Aljam3 Service interprets. Comprises (a) instructions that configure the Trigger Monitor and (b) async-function descriptors that name jobs the TM dispatches via the Queue Handler. The Behavior Contract is the workflow-as-data IR — the same artifact at compile time, run time (driving execution), and audit time (re-reading what ran). See [[spec/behavior-contract]] | Not an OTel trace — the trace is the *runtime instantiation* of a Behavior Contract; the contract is the *compiled definition*. Not pipeline source — `.jm3` files are source, the Behavior Contract is the compiler's IR output |
...
| *Agg | Canonical shorthand alias for `*Aggregate`. Aggregation collectors that reduce expand outputs to a single value (sum, count, max, etc.). Always use `*Agg` in Aljam3 Code | Not `*Aggregate` in code — `*Aggregate` is the valid long form but `*Agg` is the standard namespace used in jm3lib and EBNF |
...
| .jm3 files | Aljam3 source files written in Aljam3 Code | Not configuration files |
...
| Constructor | A `{$}` definition that produces a compile-time-guaranteed Final value with no error surface. Invoked as `$Name"literal"` in pipeline body. Two forms: string-parsing (regex captures via `($)` IO lines) and native pipeline (jm3lib only, `[-]` infallible calls). Uses `[$]` for target type binding. Metadata at `%$` | Not a pipeline — no trigger, queue, or wrapper. Not a type definition — it defines a construction path for an existing type. Not auto-derived from `{#}` |
...
|----------|-------|-------------|
| automation-builder | External | Developers who write .jm3 files |
| integrator | External | Developers who connect existing codebases via SDKs/APIs without writing .jm3 code |
...
| developer | Internal | Implementation, compile rules, jm3lib, tests |
