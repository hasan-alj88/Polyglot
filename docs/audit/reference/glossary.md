...
| Chain Step Label | A `(.)` IO line indented under `(-) $Label` in a chain call that names an individual chain step by position. Access via `>$Label.param` in chain IO lines. See [[operation-labels#Chain Step Labels]] | Not a chain-level label — `(-) $Label` labels the whole chain, `(.)` labels steps within it |
