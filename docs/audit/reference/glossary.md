...
|------|-----------|----------|
| Aljam3 | The project and platform as a whole | Not just the language |
| Aljam3 Code | The .aj3 language for workflow orchestration | Not Python/Rust/JS code used within Aljam3 |
| Aljam3 Service | Runtime backbone: Trigger Monitor + Queue Handler + Runner | Not the language itself |
...
| Behavior Contract | The compiled output of a `.aj3` file: a signal map / signal graph that the Aljam3 Service interprets. Comprises (a) instructions that configure the Trigger Monitor and (b) async-function descriptors that name jobs the TM dispatches via the Queue Handler. The Behavior Contract is the workflow-as-data IR — the same artifact at compile time, run time (driving execution), and audit time (re-reading what ran). See [[spec/behavior-contract]] | Not an OTel trace — the trace is the *runtime instantiation* of a Behavior Contract; the contract is the *compiled definition*. Not pipeline source — `.aj3` files are source, the Behavior Contract is the compiler's IR output |
...
| Chain Step Label (Retired) | Formerly a `(.)` IO line in chain calls that named individual chain steps by position. Retired with the `->` chain operator. Use operation labels (`(-) $Label`) on separate `[-]` calls instead. See [[concepts/pipelines/chains]] | Retired — chains removed from Aljam3 |
...
| Constructor | A `{$}` definition that produces a compile-time-guaranteed Final value with no error surface. Invoked as `$Name"literal"` in pipeline body. Two forms: string-parsing (regex captures via `($)` IO lines) and native pipeline (aj3lib only, `[-]` infallible calls). Uses `[$]` for target type binding. Metadata at `%$` | Not a pipeline — no trigger, queue, or wrapper. Not a type definition — it defines a construction path for an existing type. Not auto-derived from `{#}` |
...
|----------|-------|-------------|
| automation-builder | External | Developers who write .aj3 files |
| integrator | External | Developers who connect existing codebases via SDKs/APIs without writing .aj3 code |
...
| developer | Internal | Implementation, compile rules, aj3lib, tests |
