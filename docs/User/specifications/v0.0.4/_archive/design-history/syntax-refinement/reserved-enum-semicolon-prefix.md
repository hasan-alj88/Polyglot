...
```polyglot
[f] $status =? #;Boolean;True          // Syntax highlighting: both reserved

[f] $priority =? #OrderStatus.High     // Syntax highlighting: both user-defined

[f] $day =? #;DT.MyCompany;Monday     // Syntax highlighting: mixed
//             ^^           ^^^^^^
...
// Usage:
[f] $value =? #;Boolean.Extended.Maybe
```
...
// Usage:
[f] $quarter =? #;DT;Business.MyCompany.FiscalQuarters.Q1
//                  ^^^ ^^^^^^^^ ^^^^^^^^^ ^^^^^^^^^^^^^^
