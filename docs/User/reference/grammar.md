...
enum_entry ::= enum_value
alias_declaration ::= "[A]" "#" identifier
...
field_mapping ::= "[.]" dotted_field_name ":" type_annotation "<<" field_source
                  [ INDENT
                    field_mapping { field_mapping }
                    DEDENT ]

dotted_field_name ::= "." identifier { "." identifier }

field_source ::= field_accessor | enum_constant

field_accessor ::= "." identifier { "." identifier }

enum_constant ::= "#" identifier "." identifier "." identifier

serial_error_handler ::= "[s][!]" ( "!*" | custom_error_handler )

custom_error_handler ::= INDENT { statement } DEDENT
```
...
1. **Markers are enclosed in square brackets**: `[r]`, `[f]`, `[|]`
...
- Code indented under markers like `[f]`, `[!]`, `[m]` belongs to that block
- Consistent indentation is required (tabs or spaces, not mixed)

### Context Switching

- `[f]` creates a new conditional branch
...
[f] $validated =? #;Boolean;True                // Full form
   [r] $status << "ok"
```
