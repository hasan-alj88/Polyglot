...
**Statement:** Every `.jm3` file must contain exactly one `{@}` block, and it must be the first block in the file. A missing `{@}` block and multiple `{@}` blocks are both errors.
