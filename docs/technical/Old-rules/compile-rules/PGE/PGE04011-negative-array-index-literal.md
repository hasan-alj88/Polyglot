...
**Statement:** Array access with a negative literal index is a compile error. Aljam3 arrays are zero-indexed; negative indices are not valid. Reverse access must use explicit jm3lib pipelines (e.g., `-Array.Last`, `-Array.FromEnd`).
...
```aljam3
[ ] ✓ reverse access via explicit jm3lib
[-] -Array.Last
   (-) <array << $items
   (-) >item >> $last
```
