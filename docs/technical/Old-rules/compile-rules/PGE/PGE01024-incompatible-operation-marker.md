...
[ ] ✓ general pipeline used with [-]
[-] -File.Text.Read
   (-) <path << "/data/input.txt"
   (-) >content >> $text

[ ] ✓ general pipeline used with [=] (parallel)
[=] -File.Text.Read
   (-) <path << "/data/input.txt"
   (-) >content >> $text
```
...
**Open point:** jm3lib pipeline files need IO-based marker compatibility declarations added. Tracked as future work.
