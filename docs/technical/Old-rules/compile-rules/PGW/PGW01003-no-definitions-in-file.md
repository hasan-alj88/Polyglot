...
**Statement:** A `.jm3` file that contains a `{@}` package declaration but no `{#}`, `{-}`, `{W}`, `{T}`, `{Q}`, `{N}`, `{!}`, or `{_}` definitions produces a warning. Comment blocks (`{ }`) do not count as definitions.
