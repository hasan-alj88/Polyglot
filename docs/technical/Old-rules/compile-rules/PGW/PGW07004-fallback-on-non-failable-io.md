...
- For output IO (`>output >> $var` with `!> value`): resolve the parent `[-]` call. If the called pipeline has no `(-) !...` error declarations (and is not a jm3lib failable pipeline), the source is non-failable.
