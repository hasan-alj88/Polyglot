...
`{*}` is the 10th definition block type. It makes collectors ([[glossary#Reconciliation|c:Reconciliation]]) a first-class definable entity — users can define custom collector logic, not just invoke pglib-native operators.

`{N}` remains for native pipelines. `{*}` is exclusively for collector definitions. Existing pglib collectors (`*First`, `*All`, etc.) are rewritten as `{*}` blocks — the collector logic lives in `{*}`, while the underlying job management and queue handling remain native.
