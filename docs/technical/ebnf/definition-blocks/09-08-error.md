...
`{!}` defines an error tree. Each terminal leaf is typed `#Error`. The namespace uses the `!` prefix. jm3lib error namespaces (`!File`, `!No`, `!Timeout`, `!Math`, `!Validation`, `!Field`, `!Alias`, `!Permission`, `!RT`) are built-in and use `[.]` fixed leaves only.
...
**jm3lib example** (runtime-defined, fixed leaves):
```aljam3
{!} !Validation
   [.] .Schema#Error
   [.] .Type#Error
