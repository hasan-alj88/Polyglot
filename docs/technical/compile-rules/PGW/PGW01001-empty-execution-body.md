...
**Statement:** A `{-}` pipeline that has all required sections (`[T]`, `[Q]`, setup/cleanup) but no `[-]` calls in its execution body produces a warning. The pipeline is syntactically valid but performs no work.
