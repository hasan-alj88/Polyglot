...
**Statement:** Raw arithmetic tokens (`+`, `-`, `*`, `/`, `%`) are not valid operators in Aljam3 Code. Arithmetic is performed through `-Math.*` aj3lib pipelines. The compiler rejects arithmetic tokens and suggests the equivalent aj3lib pipeline.
...
**See also:** [-Math aj3lib](../../../user/aj3lib/pipelines/Math.md)
...
- [[syntax/operators|Operators]] — arithmetic through `-Math.*` aj3lib pipelines
