...
**Detection:** The compiler checks all `[_]` IO lines within permission blocks. If any line uses `>` (output direction) instead of `<` (input direction), PGE-919 fires immediately on the offending line. See [[permissions#IO Form]] for the correct input-only syntax.

**See also:** PGE-810 (IO direction mismatch — general), PGE-917 (unknown permission category), [[permissions#IO Form]]
