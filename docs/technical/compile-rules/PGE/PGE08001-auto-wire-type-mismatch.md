...
**Statement:** In wildcard auto-wire (`(-) <* << $Label>*`), the compiler pairs every output of `$Label` with exactly one input of the target pipeline via [TYPE-IDENTITY](../TYPE-IDENTITY.md). PGE08001 fires when an output type has no corresponding input type of the same type-identity (or vice versa), so no bijective mapping is possible.
