...
**Detection:** The compiler checks the parent of every `[!]` block. If the immediate parent is not an `[r]` call (or a chain `[r]`), PGE07001 fires.
...
```polyglot
[ ] ✓ chain errors scoped under the chain [r] — see PGE07002 for chain syntax
...
{=} =Process
   ...
   [r] =File.Text.Read
      [=] <path << $filepath
      [=] >content >> $content
   [!] !File.NotFound                  [ ] ✗ PGE07001 — not under [r]
      [r] $content << "fallback"
```

```polyglot
[ ] ✗ PGE07001 — [!] under [?] conditional, not under [r]
[?] $mode
   [?] "read"
      [r] =File.Text.Read
         [=] <path << $filepath
         [=] >content >> $content
   [!] !File.NotFound                  [ ] ✗ PGE07001 — parent is [?], not [r]
      [r] $content << "fallback"
   [?] *?
      [r] =NoOp
```
