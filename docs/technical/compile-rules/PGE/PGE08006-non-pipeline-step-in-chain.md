...
{=} =BadFirst
   ...
   [W] =W.Polyglot
   [=] >out#string
   [r] =Path"/tmp"=>=Process            [ ] ✗ PGE08006 — value expr in chain
      [=] >result >> >out
```
...
{=} =BadMiddle
   ...
   [=] <data#string
   [=] >out#string
   [r] =Parse=>=Path"/tmp"=>=Store      [ ] ✗ PGE08006 — value expr in chain
      [=] <input << $data
      [=] >result >> >out
```
...
{=} =BadLast
   ...
   [=] <data#string
   [=] >out#path
   [r] =Parse=>=Path"/tmp"              [ ] ✗ PGE08006 — value expr in chain
      [=] <input << $data
      [=] >result >> >out
```
...
{=} =BadInterp
   ...
   [=] <dir#string
   [=] >out#string
   [r] =Fetch=>=Path"{$dir}/output"=>=Store        [ ] ✗ PGE08006 — value expr in chain
      [=] <url << $dir
      [=] >result >> >out
```
