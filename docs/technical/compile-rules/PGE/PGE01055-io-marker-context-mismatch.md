...
**Rationale:** Aljam3 syntax is designed to be highly readable. Using an IO marker that belongs to a different block type (e.g., using a Pipeline IO marker inside a Data block) creates syntactic dissonance and breaks the language's strict visual parsing rules.
