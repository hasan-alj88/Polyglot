...
**Statement:** A `[_]` permission declaration must use a known category and subfield. Polyglot defines exactly 8 permission categories with fixed subfields (see [[permissions#Permission Categories]]). If a `[_]` declaration references a category or subfield not in the predefined set, PGE-917 fires.
...
**INVALID:**
```polyglot
[ ] ✗ PGE-917 — unknown category "Network"
{=} =SendData
   [_] _Network.send"*"                       [ ] ✗ PGE-917 — _Network is not a known category
   [t] =T.Manual
   [Q] =Q.Default
   [W] =W.Polyglot
...
```polyglot
[ ] ✗ PGE-917 — unknown subfield "rename" under known category File
{=} =RenameFile
   [_] _File.rename"/tmp/*"                   [ ] ✗ PGE-917 — _File.rename is not a known subfield
   [t] =T.Manual
   [Q] =Q.Default
   [W] =W.Polyglot
...
```polyglot
[ ] ✗ PGE-917 — unknown subfield "delete" under known category Web
{=} =CleanupEndpoint
   [_] _Web.delete"https://api.example.com/*" [ ] ✗ PGE-917 — _Web.delete is not a known subfield
   [t] =T.Manual
   [Q] =Q.Default
   [W] =W.Polyglot
