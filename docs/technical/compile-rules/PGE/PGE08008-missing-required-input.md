...
**Statement:** Every declared `<input` on the called pipeline or operator that has no default value (`<~`) must be wired by the caller. If the caller omits a required input, PGE08008 fires. This applies to sequential pipeline (`[r]`), parallel pipeline (`[p]`), expand operator (`[~]`), collect operator (`[*]`), and wrapper (`[W]`) calls.
