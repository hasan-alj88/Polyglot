...
**See also:** PGE-401 (type mismatch), PGE-808 (missing required input)
...
**INVALID:**
```polyglot
[ ] ✗ PGE-416 — literal matches constraint
[r] =Math.Divide
   [=] <numerator << $total
   [=] <denominator << 0                      [ ] ✗ PGE-416 — 0 is in constraint set {"0"}
   [=] >result >> $result
```
...
[r] =Math.Modulo
   [=] <numerator << $total
   [=] <denominator << 0                      [ ] ✗ PGE-416 — modulo by zero
   [=] >result >> $remainder
```
