...
**Detection:** The compiler tracks which fields belong to fixed (`.`) schema levels. Any attempt to access or assign a `.`-separated field not declared in the `{#}` definition triggers PGE-404. For `:` fields, no such check is performed — they are open by design.
