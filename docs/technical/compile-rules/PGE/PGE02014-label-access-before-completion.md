...
**Rationale:** A label's outputs are only populated once the labeled operation finishes. Accessing them beforehand would read uninitialized state, which Polyglot forbids (see PGE02002). The compiler enforces this statically by analyzing execution ordering. This is Polyglot's compile-time approach to preventing race conditions — in a system where operations may execute concurrently, the compiler proves that data dependencies are satisfied before access, eliminating a class of timing bugs that would only surface at runtime.
