...
**Detection:** The compiler counts the dot-separated integer indices in an element access expression and compares against the declared `:<N>D` dimension. If the counts differ, PGE-417 fires. Also fires if `:0D` is declared.

**See also:** PGE-412 (nested array type), PGE-411 (negative array index literal)
...
**INVALID:**
```polyglot
[ ] ✗ PGE-417 — too many indices for :2D array
[=] <matrix#array:float:2D
[r] $val << $matrix.0.1.2                    [ ] ✗ 3 indices on :2D — expected 2
```

```polyglot
[ ] ✗ PGE-417 — too few indices for :3D array
[=] <cube#array:int:3D
[r] $val << $cube.2                           [ ] ✗ 1 index on :3D — expected 3
```

```polyglot
[ ] ✗ PGE-417 — :0D is not a valid dimension
