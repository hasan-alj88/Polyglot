...
{-} -BadPipe
   ...
   [Q] -Q.Default
   [W] -W.Polyglot
   [-] $data << -File.Text.Read >> "/var/log/app.log"  [ ] ✗ PGE10004 — no {_} object grants File.Read
```
...
{-} -Outer
   ...
   [Q] -Q.Default
   [W] -W.Polyglot
   [-] $result << -Inner                               [ ] ✗ PGE10004 — -Outer calls -Inner which uses File.Read, but -Outer has no [_] granting File.Read
```
