...
**Rationale:** Division by zero is always a runtime failure. When the divisor is a literal `0`, the compiler can detect this statically and reject the program. This catches obvious mistakes before deployment. This exemplifies Aljam3's core principle of offloading runtime crashes to compile time — if a failure is provable before the pipeline runs, the compiler must reject it rather than let it surface in production.
