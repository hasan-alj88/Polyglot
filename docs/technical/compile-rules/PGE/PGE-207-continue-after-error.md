...
**Rationale:** The default `[!]` behavior terminates the pipeline on error (see PGE-205). `*Continue` provides an explicit opt-in to keep the pipeline running by supplying a known-good fallback value. No boolean flag is needed — the variable's lifecycle state (Final vs Failed) naturally controls whether downstream pipelines fire.
...
[r] =Fetch
   [=] >payload >> >data
   [!] !FetchError
      [*] *Continue               [ ] ✗ PGE-207 — must provide >FallBack
```
