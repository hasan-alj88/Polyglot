...
**Rationale:** Parallel operations spawn multiple concurrent instances. A label inside a parallel scope refers to one instance per parallel branch — there is no single deterministic value to read. The collector must aggregate or select results before they become available to sequential code, maintaining Aljam3's pull isolation guarantee.
