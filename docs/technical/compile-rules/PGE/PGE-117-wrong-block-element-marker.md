...
**See also:** PGE-116 (Unmarked Execution Line — missing marker entirely), PGE-102 (IO Before Trigger — IO placement constraints)
...
**INVALID:**
```polyglot
[ ] ✗ PGE-117 — [=] used for standalone assignment in execution body
{=} =Bad
   [t] =T.Manual
   [Q] =Q.Default
   [W] =W.Polyglot
...
```polyglot
[ ] ✗ PGE-117 — [=] used for output push inside conditional
{=} =AlsoBad
   [t] =T.Manual
   [Q] =Q.Default
   [W] =W.Polyglot
   ...
   [?] *?
      [=] >output << "error"                  [ ] ✗ PGE-117 — should be [r]
```
