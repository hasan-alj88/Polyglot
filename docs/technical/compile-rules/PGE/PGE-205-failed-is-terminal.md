...
**Detection:** At any push statement that targets a variable whose producing `[r]` call has an `[!]` error path that does not itself provide a replacement value or `*Continue` fallback. At runtime, if the producing pipeline fails and a downstream pipeline attempts to pull, the runtime fires PGE-205.

#### Default behavior: `[!]` ends the pipeline

When an `[!]` error handler does NOT push a replacement value into the output variable and does NOT use `[*] *Continue`, the pipeline **terminates on error**. No downstream code runs. This is the safe default — Failed state never reaches downstream code. The compiler emits PGW-205 to alert the developer about this termination behavior.
...
2. **`*Continue` fallback** — use `[*] *Continue >FallBack << "value"` inside `[!]`. See [PGE-207 — Continue After Error](PGE-207-continue-after-error.md).
...
[r] =Fetch
   ...
   [!] !FetchError
      [r] =LogError
         [=] <msg << "fetch failed"
      [ ] pipeline ends here on error — default behavior (PGW-205 warns)
[r] =Process
...
[r] >result << "override"          [ ] ✗ PGE-205 — >result may be Failed, cannot push
```
