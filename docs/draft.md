*All polyglot object are serlized data *
The truth is mush deeper than that, they all in one big serlized data in NoSQL server. 

for example #Boolean.True full path is .Data.#:Boolean.True (also notice the `:` means you may add as many `{#}` definations as you need). a pipeline like =T.Daily full path is .pipeline:T.Daily

Aliasing works by adding refrence earlier in the full path tree like #True is refrences #Boolean.True and full path of #True is .Data.#:True  is there is duplicate or name clash there then there should be PGE compile error.
...
**Topic:** #DateTime data tree + =DT.* stdlib pipelines
**Target:** Already saved to `docs/technical/brainstorming/datetime-tree.md`. When formalized: `#DateTime` types → `docs/user/STDLIB.md` section 7, `=DT.*` pipelines → section 1.
**Contradictions:** None — this is entirely new content. No existing `=DT.*` or `#DateTime` in STDLIB.md.

**Gaps found vs existing STDLIB patterns:**

1. **Missing `[%]` metadata** — All existing stdlib types (`#Boolean`, `#OS`, `#path`) have `[%] .description`, `[%] .version`, `[%] .alias` blocks. The draft's `#DateTime` and all supporting types have none.
2. **Missing error definitions** — Existing stdlib pipelines define their errors (e.g. `=File.Text.Read` → `!File.NotFound`, `!File.ReadError`). The draft's `=DT.*` pipelines don't define any `!DT.*` errors.
3. **`#WeekSystem` enum-with-data pattern** — The draft has `.ISO;BusinessWeek` which makes enum variants carry typed data. This is valid per existing patterns (e.g. `#Severity.Critical` with `.code;int`), but `#WeekSystem` mixes it differently — each variant IS a `BusinessWeek`. Need to confirm this is the intended pattern vs having a separate `.business` field reference.

### Questions

1. Should metadata (`[%]` blocks) and error trees (`!DT.*`) be added now in the brainstorm, or deferred to formal spec phase?
   > _answer here_

2. The serialized data model notes at the top (lines 1-6) — should those be saved separately to `docs/technical/brainstorming/` as their own file, or left in draft for a future session?
   > _answer here_

3. The brainstorm is already saved to `docs/technical/brainstorming/datetime-tree.md`. Is there anything else to place, or is this session done?
   > _answer here_