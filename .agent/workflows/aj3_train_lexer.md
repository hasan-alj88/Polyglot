...
1. **Phase 1: Lexical Analysis**
   - Apply the lexer to Aljam3 code (either from the `jm3_train_code` workflow or from an existing Aljam3 file).
   // turbo-all
...
3. **Phase 3: Acceptance**
   - If the user **accepts**:
     - Save the valid `.jm3ts` token stream file to `lib/aljam3/tests/fixtures/token_streams/valid/`.
...
4. **Phase 4: Correction & Documentation**
   - If the token stream is **incorrect**:
     ...
     - Save the incorrect `.jm3ts` token stream (with comments explaining why it is wrong) to `lib/aljam3/tests/fixtures/token_streams/invalid/`.
     ...
     - Save the final, corrected `.jm3ts` token stream to `lib/aljam3/tests/fixtures/token_streams/valid/`.
     - Close the workflow or start another round.
</process>
