---
description: Train the model on Polyglot code syntax and semantics.
---

<objective>
Generate Polyglot code scenarios, review with the user, and add to valid or invalid examples to train the model's understanding of the language syntax.
</objective>
...
1. **Phase 1: Generation**
   ...
   - Generate a complete Polyglot (`.pg`) file based on a random scenario if the user has not specified one.
   - Save the generated file to the disk in a temporary location (e.g., `lib/polyglot/tests/fixtures/qa/test_qa.pg`) for review.
...
3. **Phase 3: Acceptance & Documentation**
   - If the user **accepts** the code:
     - Move or save the accepted code into the valid examples folder (`lib/polyglot/tests/fixtures/examples/valid/`).
   ...
   - If the code is **incorrect**:
     ...
     - Save the original incorrect code (with comments explaining why it is wrong) into the invalid examples folder (`lib/polyglot/tests/fixtures/examples/invalid/`).
     - Save the corrected version of the code to the valid examples folder (`lib/polyglot/tests/fixtures/examples/valid/`).
