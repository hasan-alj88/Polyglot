...
| Issue-Driven | issues, work-issue, merge |
...
Work on individual GitHub issues with branch isolation and user-controlled merging.

### Workflow

```
/paul:issues              # Browse open issues grouped by label
/paul:work-issue 36       # Select issue, create branch, research
/paul:plan                # Create plan (reads issue context from STATE.md)
/paul:apply               # Execute the plan
/paul:unify               # Close PAUL loop (SUMMARY.md)
/paul:merge               # Merge branch to main with conflict control
```

### `/paul:issues`
Browse open GitHub issues grouped by type label.

- Fetches issues via `gh issue list`
- Groups by label: compiler, algorithm, stdlib, design, test, docs
- Sub-sorts by priority (P1 first)
- Displays formatted dashboard with counts

Usage: `/paul:issues`
Usage: `/paul:issues --label stdlib`

---

### `/paul:work-issue <number>`
Select an issue, create a branch, and research context.

- Fetches full issue details from GitHub
- Creates branch: `{type}/issue-{number}-{slug}`
- Researches codebase for related PGE/PGW codes, files, issues
- Updates STATE.md with Active Issue section
- Enters discussion mode for further exploration

Usage: `/paul:work-issue 36`

---

### `/paul:merge`
Merge issue branch to main with user-controlled conflict resolution.

- Validates UNIFY is complete before merging
- Offers merge strategy: merge commit, squash, or rebase
- Interactive per-conflict resolution (keep main, keep branch, edit manually)
- Closes GitHub issue
...
*PAUL Framework v0.5 | 29 commands | Last updated: 2026-03-24*
</reference>
