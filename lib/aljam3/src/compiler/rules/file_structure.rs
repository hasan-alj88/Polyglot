...
impl Rule for FileStructureAlgorithm {
    fn validate(&self, ctx: &AnalysisContext, report: &mut ValidationReport) {
        ...
        for (i, spanned_token) in ctx.tokens.iter().enumerate() {
            if matches!(spanned_token.value, Aljam3Token::DefPackage) {
                package_defs += 1;
                if package_defs > 1 {
                    report.add_error(ValidationError {
                        ...
                        message: "A single Aljam3 file can only contain one package definition `{@}`.".to_string(),
            ...
            if first_non_comment_idx.is_none() {
                match spanned_token.value {
                    Aljam3Token::DefComment | Aljam3Token::CommentText(_) | Aljam3Token::TokSpace => {}
                    _ => {
        ...
        if package_defs == 0 {
            report.add_error(ValidationError {
                ...
                help: Some("All Aljam3 files must start with a package definition `{@}`.".to_string())
            });
        } else if let Some(first_idx) = first_non_comment_idx {
            if !matches!(ctx.tokens[first_idx].value, Aljam3Token::DefPackage) {
                report.add_error(ValidationError {
                    context_snippets: vec![],
                    code: "PGE01001".to_string(),
                    name: "One Package Per File".to_string(),
                    message: "The first statement in a Aljam3 file must be a package definition `{@}`.".to_string(),
