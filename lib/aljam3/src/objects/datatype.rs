use super::tree::{Aljam3DataTree, Aljam3DataState, Aljam3DataLeaf, Aljam3LeafValue};
...
impl DatatypeComparator {
    /// Compares two Aljam3DataTrees directionally.
    /// Evaluates if the `target` theoretical topology can match the `source` topology.
    pub fn is_compatible(target: &Aljam3DataTree, source: &Aljam3DataTree) -> ComparisonResult {
        // 1. Check if source is instantiated (Final or Default)
        let source_state = source.state();
        if source_state != Aljam3DataState::Final && source_state != Aljam3DataState::Default {
        ...
        for key in target_keys {
            ...
            let t_val = match target_leaf {
                Aljam3DataLeaf::Default(v) | Aljam3DataLeaf::Final(v) => Some(v),
            ...
            if let (Some(t), Some(s)) = (t_val, s_val) {
                match (t, s) {
                    (Aljam3LeafValue::String { .. }, Aljam3LeafValue::String { .. }) => {},
                    (Aljam3LeafValue::Enum, Aljam3LeafValue::Enum) => {},
                    _ => {
                        return ComparisonResult::Mismatch(format!(
