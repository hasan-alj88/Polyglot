...
pub fn detect_cycles(tokens: &[Spanned<PolyglotToken>]) -> Vec<Vec<String>> {
    ...
    while i < tokens.len() {
        match &tokens[i].value {
            PolyglotToken::DefPipeline => {
                if i + 1 < tokens.len() {
                    if let PolyglotToken::Pipeline(name) = &tokens[i+1].value {
            ...
            PolyglotToken::ActionExecSeq | PolyglotToken::ActionExecPar | PolyglotToken::ActionExecBg => {
                if let Some(parent) = &current_pipeline {
                    if i + 1 < tokens.len() {
                        if let PolyglotToken::Pipeline(target) = &tokens[i+1].value {
                            // Exclude pglib standard pipelines
