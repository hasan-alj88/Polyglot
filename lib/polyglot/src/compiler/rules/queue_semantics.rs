        ...
        while i < ctx.tokens.len() {
            ...
            if let PolyglotToken::DefQueue = token_val {
                if i + 1 < ctx.tokens.len() {
                    if let PolyglotToken::QueueConfig(name) = &ctx.tokens[i+1].value {
                        ...
                        while j < ctx.tokens.len() {
                            ...
                            if let PolyglotToken::Scope(s) = child_token.value {
                                ...
                                if s == 1 {
                                    ...
                                    if j + 1 < ctx.tokens.len() && matches!(ctx.tokens[j+1].value, PolyglotToken::DataInput) {
                                        if j + 2 < ctx.tokens.len() {
                                            if let PolyglotToken::DataField(field_name) = &ctx.tokens[j+2].value {
                                                // Extract value assigned to the field
                                                let mut k = j + 3;
                                                let mut value = String::new();
                                                while k < ctx.tokens.len() && !matches!(ctx.tokens[k].value, PolyglotToken::Scope(_)) {
                                                    if let PolyglotToken::PushInto = ctx.tokens[k].value {
                                                        if k + 1 < ctx.tokens.len() {
                                                            match &ctx.tokens[k+1].value {
                                                                PolyglotToken::StringLiteral(v) => value = v.clone(),
                                                                PolyglotToken::Data(v) => value = v.clone(),
                                                                _ => {}
                                                            }
                                                        }
                                                    }
        ...
        while i < ctx.tokens.len() {
            ...
            if let PolyglotToken::DefPipeline = token_val {
                ...
                while j < ctx.tokens.len() {
                    ...
                    if let PolyglotToken::ActionQueue = child_token.value {
                        in_q_block = true;
                    }

                    if in_q_block {
                        // Detect queue assignment: -Q.Assign"QueueName"
                        if let PolyglotToken::Pipeline(p_name) = &child_token.value {
                        ...
                        if let PolyglotToken::InputParameter(param_name) = &child_token.value {
                                ...
                                if let Some(constraints) = queue_registry.get(&q_name) {
                                    ...
                                    while k < ctx.tokens.len() && !matches!(ctx.tokens[k].value, PolyglotToken::Scope(_)) {
                                        if let PolyglotToken::PushInto = ctx.tokens[k].value {
                                            if k + 1 < ctx.tokens.len() {
                                                match &ctx.tokens[k+1].value {
                                                    PolyglotToken::StringLiteral(v) => local_value = v.clone(),
                                                    PolyglotToken::Data(v) => local_value = v.clone(),
                                                    _ => {}
                                                }
                                            }
                                        }
