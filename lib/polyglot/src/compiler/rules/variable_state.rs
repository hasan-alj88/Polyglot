    ...
    fn validate(&self, ctx: &AnalysisContext, report: &mut ValidationReport) {
        ...
        while i < ctx.tokens.len() {
                    ...
                    if !found {
                        ...
                        while j < ctx.tokens.len() {
                            match &ctx.tokens[j].value {
                                PolyglotToken::TokSpace => { j += 1; continue; }
                                PolyglotToken::DataType(dt) => {
                                    has_datatype = true;
            ...
            if let Some((op_type, dir)) = get_operator_info(&token.value) {
                ...
                for j in (0..i).rev() {
                    match &ctx.tokens[j].value {
                        PolyglotToken::TokSpace | PolyglotToken::DataType(_) => continue,
                        PolyglotToken::Variable(name) => {
...
fn get_operator_info(token: &PolyglotToken) -> Option<(OpType, Direction)> {
    match token {
        PolyglotToken::PullFrom => Some((OpType::FinalPush, Direction::RightToLeft)), // <<
        PolyglotToken::PushInto => Some((OpType::FinalPush, Direction::LeftToRight)), // >>
        PolyglotToken::DefaultPullFrom => Some((OpType::DefaultPush, Direction::RightToLeft)), // <~
        PolyglotToken::DefaultPushInto => Some((OpType::DefaultPush, Direction::LeftToRight)), // ~>
        PolyglotToken::FallBackPullFrom => Some((OpType::FallbackPush, Direction::RightToLeft)), // !<
        PolyglotToken::FallBackPushInto => Some((OpType::FallbackPush, Direction::LeftToRight)), // !>
        _ => None,
    }
}
...
mod tests {
    ...
    fn test_context(tokens: Vec<PolyglotToken>) -> ValidationReport {
        ...
        let mut report = ValidationReport::new("test.pg".to_string());
    ...
    fn test_valid_transitions() {
        let tokens = vec![
            PolyglotToken::Scope(0),
            PolyglotToken::Variable("$var".to_string()),
            PolyglotToken::DataType("#int".to_string()),
            PolyglotToken::DefaultPullFrom, // <~
            PolyglotToken::Data("1".to_string()),
            
            PolyglotToken::Variable("$var".to_string()),
            PolyglotToken::PullFrom, // <<
            PolyglotToken::Data("2".to_string()),
        ];
    ...
    fn test_pge02002_pull_from_declared() {
        let tokens = vec![
            ...
            PolyglotToken::DataType("#int".to_string()),
            // Declared
            PolyglotToken::Variable("$other".to_string()),
            PolyglotToken::DataType("#int".to_string()),
            PolyglotToken::PullFrom, // <<
            PolyglotToken::Variable("$var".to_string()), // Source
        ];
        let report = test_context(tokens);
        assert!(!report.violations.is_empty());
    ...
    fn test_pge02003_push_to_final() {
        let tokens = vec![
            ...
            PolyglotToken::Variable("$var".to_string()),
            PolyglotToken::DataType("#int".to_string()),
            PolyglotToken::PullFrom, // << (Final push to $var)
            PolyglotToken::Data("1".to_string()),
            
            PolyglotToken::Variable("$var".to_string()),
            PolyglotToken::PullFrom, // << (Another final push to $var)
            PolyglotToken::Data("2".to_string()),
        ];
        let report = test_context(tokens);
        assert!(!report.violations.is_empty());
