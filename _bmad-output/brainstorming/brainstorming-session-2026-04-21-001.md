...
_Concept:_ Users can define `{$}` constructors. Only pglib constructors can use pipeline calls to infallible native pipelines inside `{$}`. User constructors are pure regex-to-tree mapping only.
...
- Only pglib can use `[-]` pipeline calls inside `{$}`
