...
**Context:** Designed native dispatch spec, created lib/ scaffold, renamed stdlib→pglib, began pglib .pg source files
...
- Created **lib/Polyglot/pglib/** with actual `.pg` source files organized by subsystem kind:
...
- **Renamed stdlib → pglib** — physical folder move + ~190 text replacements across 70+ files, file renames (STDLIB.md → PGLIB.md, PGE09012, phase folders, memory files)
...
| stdlib renamed to pglib | "pglib" = Polyglot library, clearer identity | 190+ occurrences updated; folder moved; all wikilinks updated |
| pglib .pg source files live in `lib/Polyglot/pglib/` | Organized by subsystem kind (tm/, qh/, runner/, intrinsics/) | Compiler reads these as source of truth for native definitions |
...
**Notes:** User wants `docs/user/pglib/pipelines/DT.md` (692 lines, 37 pipelines) split into `DT/` folder with one `.md` per pipeline definition
**Effort:** Medium — 37 files + INDEX.md + wikilink updates
**Reference:** `@docs/user/pglib/pipelines/DT.md`
...
**Reference:** `@docs/user/pglib/pipelines/`
...
**Reference:** `@docs/user/pglib/types/NativeKind.md:65-71`, `@docs/user/concepts/pipelines/INDEX.md:132-138`, `@docs/technical/compile-rules/PGE/PGE01028-base-derived-mutual-exclusion.md`

### DT.md `[%] .version` Lines in {N} Blocks
**Status:** REVIEW
**Notes:** The .pg files in `lib/Polyglot/pglib/runner/datetime.pg` don't include `.version` metadata. DT.md had `[%] .version << "1.0.0"` on every {N} block. Need to decide: is `.version` valid on {N} blocks? EBNF §9.4c `native_field` only lists `.Kind`, `.<Language>`, `.description`.
...
2. **GitHub issue labels**: What labels to use? (e.g., `pglib`, `native-impl`, `rust`, `spec`)
...
```
@docs/user/pglib/pipelines/DT.md              — file to split
@docs/user/pglib/pipelines/File.md             — template candidate
@docs/user/pglib/pipelines/Math.md             — template candidate
@docs/user/pglib/pipelines/Q.md                — template candidate
@docs/user/pglib/pipelines/T.md                — template candidate
@docs/user/pglib/pipelines/W.md                — template candidate
@docs/user/pglib/pipelines/#.md                — template candidate
@docs/user/pglib/pipelines/RT.md               — template candidate
@docs/technical/spec/native-dispatch.md         — dispatch spec (just written)
@lib/Polyglot/pglib/                           — .pg source files
...
**Current:** Post v0.2 — native dispatch architecture designed, lib/ scaffold created, pglib rename complete
