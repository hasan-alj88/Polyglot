...
| `docs/user/stdlib/errors/errors.md` | Modified | Tree path ref to `.`, added `!Error` namespace section |
...
| Error namespaces `.` fixed | Stdlib namespaces are Polyglot-defined; only `!Error` is user-extensible | `.Error` is fixed name, children are `:` flexible |
