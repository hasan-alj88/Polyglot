...
tags: [permissions, packages, pipelines, ceiling, PGE-915, PGE-916]
...
provides:
  ...
  - "PGE-915 and PGE-916 compile rules"
...
key-decisions:
  - "PGE-915 for pipeline exceeds package ceiling, PGE-916 for imported package exceeds importer ceiling (two separate rules)"
...
**Added package permission ceiling to {@} block spec and pipeline-level [_] declaration to pipeline spec, with PGE-915/916 compile rules.**
...
| AC-4: Compile Rules Complete | Pass | PGE-915 and PGE-916 in compile rules table |
...
- Added PGE-915 (pipeline exceeds ceiling) and PGE-916 (imported package exceeds importer) to compile rules
...
| File | Change | Purpose |
|------|--------|---------|
| `docs/user/syntax/packages.md` | Modified | Added Permissions section + PGE-915/916 table entries |
...
**Concerns:**
- PGE-915/916 detail files not yet created (table entries only)
