...
| `docs/user/pglib/collectors/Agg.md` | Line 10: warns `*Agg NOT *Aggregate` but no glossary entry backs this |
| `docs/technical/spec/metadata-tree/object-types.md` | Line 19: `%*` branch lists `*Agg.*` — confirms it as a core namespace |

## Example
**Source A** (`docs/user/pglib/collectors/Agg.md`, line ~10):
...
## Recommendation
Add glossary entries for at least the operator prefix categories: `*` (Collector), `~` (Expander), and specifically `*Agg` with the "NOT *Aggregate" warning. Vision.md likely does not need changes since it is a high-level philosophy document, but the glossary as the authoritative term reference should cover the pglib namespace shorthands. Consider whether `*Into`, `*All`, `*First`, `*Nth`, and `*Continue` also need entries.
