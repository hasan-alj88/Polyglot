...
plan: 03
subsystem: docs
tags: [native, stdlib, DT, queue, propagation]
...
# Issue #121 Plan 03: Propagate {N} to Stdlib — Summary

**Converted 44 stdlib pipeline definitions from old `{=}`/`{Q}` + `.baseCode` syntax to `{N}` blocks with `%Native.*` metadata, and aligned all "base pipeline" terminology to "native definition".**
...
- 121-03: Propagate {N} to stdlib (DT.md, queue.md, io-triggers.md, #.md)
