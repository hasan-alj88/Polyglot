...
<objective>
## Goal
Update compile rules and stdlib permission references to reflect the {_} named permission object system designed in Plans 01-02.

## Purpose
Plans 01-02 rewrote user-facing docs and technical spec (EBNF + metadata tree) for {_} objects. Compile rules and stdlib files still use the old inline `[_] _Category.subfield` syntax. This plan brings them into alignment.

## Output
- 5 compile rule files rewritten for {_} object system
- 7 stdlib pipeline files with updated permission sections
</objective>
...
## AC-2: Stdlib permission tables use Category.Capability format
```gherkin
Given stdlib files File.md, W.md, T.md, DT.md, Sys.md, RT.md, #.md
...
And [_] _IO.Read on =DT.Now is removed (stdlib {N} defs don't have [_] — they require capabilities implicitly)
...
<task type="auto">
  ...
  <action>
    ...
    **PGE10004 — Undeclared Permission:**
    - Statement: pipeline calls IO stdlib without matching `[_] _ObjectName` reference whose `{_}` object grants the needed capability
...
<task type="auto">
  <name>Task 2: Update 7 stdlib files' permission sections</name>
  <files>
    docs/user/stdlib/pipelines/DT.md,
    docs/user/stdlib/pipelines/File.md,
    docs/user/stdlib/pipelines/W.md,
  ...
  <action>
    Update each stdlib file's Permissions section:
    ...
    **#.md — minimal change:**
    ...
    - Adding {_} definition blocks to stdlib files (these are reference docs, not example files)
  ...
  <done>AC-2, AC-3, AC-4 satisfied: stdlib files use new permission terminology and syntax</done>
</task>
...
- Do not change stdlib pipeline definitions, IO, errors, or descriptions — only Permissions sections
...
- Compile rules and stdlib are fully aligned with {_} object system from Plans 01-02
</success_criteria>
