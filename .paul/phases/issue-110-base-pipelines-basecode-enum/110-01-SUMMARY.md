...
plan: 01
subsystem: docs
tags: [base-pipeline, derived-pipeline, baseCode, aj3lib, compile-rule]
...
- Created `#BaseCode` enum type doc with full Rust variant tree mirroring all aj3lib pipeline names
...
| File | Change | Purpose |
|------|--------|---------|
| `docs/user/aj3lib/types/BaseCode.md` | Created | `#BaseCode` enum definition, variant tree, config, usage |
...
- `#BaseCode` enum available for aj3lib pipeline files (future issue)
- PGE01028 ready for compiler implementation

**Concerns:**
- aj3lib pipeline `.pg` files don't yet declare `[%] .baseCode` — separate issue needed
