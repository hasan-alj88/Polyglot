...
4. **`%##Depth.Max` Inference Model** — updated to use `%##` prefix, added PGE11002 and PGW11003 references
5. **`###` Field Types** — new subsection documenting ###Value and ###Enum with inference rules, PGE11003 (contradiction), PGE05005 (mixed sibling kinds), examples from #Boolean and #String
...
7. **#IndexString** — new Layer 2c subsection with {#} definition, regex `^[^\s.<>:]+$`, PGE11004 documentation
...
13. **Enum Fields vs Value Fields** — added `###` kind column, PGE05005 reference, cross-reference to field types section
...
- [x] AC-3: ###Value and ###Enum documented with inference, PGE11003, PGE05005
- [x] AC-4: `<` operator dual role documented with type param defaults
- [x] AC-5: #IndexString added with {#} definition and PGE11004
