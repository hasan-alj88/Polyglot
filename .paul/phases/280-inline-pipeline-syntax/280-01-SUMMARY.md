...
plan: 01
subsystem: docs
tags: [inline-calls, %InlineString, template, compiler-rules, pglib]
...
provides:
  ...
  - "3 pglib pipelines updated to use %InlineString"
affects: [compile-rules, pglib, inline-calls]
...
| AC-4: pglib Pipelines Use %InlineString | Pass | -Path, -T.Daily, -T.Webhook all declare %InlineString with named inputs |
...
- Updated 3 pglib pipelines (-Path, -T.Daily, -T.Webhook) and 3 edge case entries (EC-4.9, EC-4.11, EC-4.12) to use new syntax
...
| docs/user/pglib/pipelines/Path.md | Modified | %InlineString << "{path}", <path#path |
| docs/user/pglib/pipelines/T/Daily.md | Modified | %InlineString << "{time}", <time#string |
| docs/user/pglib/pipelines/T/Webhook.md | Modified | %InlineString << "{endpoint}", <endpoint#string |
