...
Add text and CSV collection operators to jm3lib, completing the expand/collect pattern for text data. Includes:
...
- The k-way merge collector takes #TextDiffs from a separate =Text.Diff comparator pipeline — the comparator is its own jm3lib pipeline, not baked into the collector
